...
> **⚠️ MindsHub REMOVED (2026-07-08):** MindsHub/Minds Cloud is no longer a provider.
> Users bring their own API key (Anthropic/OpenAI/Gemini) or use CLI agents (Claude Code).
> - `minds_api_key`, `minds_url`, `minds_cloud` no longer exist in settings/models.
> - Publishing uses `ANTON_MINDS_API_KEY` env var (not a settings field).
> - `Provider` enum has: `ANTHROPIC`, `OPENAI`, `GEMINI`, `OPENAI_COMPATIBLE` only.
> - Frontend provider types: `anthropic`, `openai`, `gemini`, `openai-compatible`.
>
> See `.minds/war_room.md` "MindsHub Stripped" entry for full file-by-file breakdown.
