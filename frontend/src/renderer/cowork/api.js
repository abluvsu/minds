...
// owned by the backend (cowork-server). The live `/v1/models` set. Returns null on failure so callers fall back to
...
// ── Provider registry — the user's configured model sources ─────────
//
// Each entry is one API key + base URL + model list, keyed by a stable
// slug (see cowork.services.provider_registry on the backend). Distinct
// entries let the same provider `type` (e.g. "gemini") appear more than
// once with different keys — this is how two free-tier accounts become
// two independent, failover-able model sources.

export async function fetchProviderRegistry() {
  try {
    return await req('/providers/');
  } catch {
    return [];
  }
}

export async function createProviderEntry(entry) {
  return req('/providers/', { method: 'POST', body: JSON.stringify(entry) });
}

export async function updateProviderEntry(slug, patch) {
  return req(`/providers/${encodeURIComponent(slug)}`, { method: 'PUT', body: JSON.stringify(patch) });
}

export async function deleteProviderEntry(slug) {
  return req(`/providers/${encodeURIComponent(slug)}`, { method: 'DELETE' });
}

export async function pingProviderEntry(slug) {
  return req(`/providers/${encodeURIComponent(slug)}/ping`, { method: 'POST' });
}

export async function fetchProviderEntryModels(slug) {
  try {
    const r = await req(`/providers/${encodeURIComponent(slug)}/models`);
...
// Flattens the registry into composer-ready picker options: one entry
// per (provider, model) pair, id = "{slug}/{model}" — the exact string
// build_llm_client() on the backend expects back as `request.model`.
export async function fetchModelPickerOptions() {
  const rows = await fetchProviderRegistry();
  const options = [];
  for (const row of rows) {
    if (!row.enabled || !Array.isArray(row.models)) continue;
    for (const modelId of row.models) {
      options.push({
        id: `${row.slug}/${modelId}`,
...
export async function fetchSettings() {
  const op = _settingsLock.then(async () => {
    try {
      ...
      // Overlay the server's recommended-models (live `/v1/models`).
      // Falls back to the static lists seeded by
...
export async function pollOAuthStatus(service) {
  return req(`/connectors/oauth/${encodeURIComponent(service)}/status`);
}
...
export const MOCK_DATA = {
  tasks: [
    ...
    { id: 't5', title: 'Create website copy for Cowork', subtitle: '2 weeks ago', status: 'done', messages: [
      { role: 'user', content: 'Create website copy for Cowork' },
      { role: 'assistant', content: 'Done — copy is in your Artifacts.' },
    ]},
  ...
  projects: [
    ...
    { id: 'p3', name: 'Cowork brand', description: 'Brand and identity for the Cowork app', taskCount: 6, fileCount: 12, updated: '3d ago', tint: 'rgba(120,186,172,0.18)', color: 'var(--sage-700)' },
