...
const PLUGIN_DIRECTORY_CARDS = [
  { name: 'Box', vendor: 'Box', desc: 'Work with your Box content directly from Cowork - search files, organize folders, collaborate with your team.' },
...
function ConnectorsPage({
  ...
  if (driveSelected) {
    ...
    detailNotes.push(`Setup for ${selectedConnector?.name} is not wired into Cowork yet.`);
  }
        ...
        <div className="customize-empty-detail">
          ...
          <div className="customize-empty-title">
            {selectedConnector?.status === 'included'
              ? `${selectedConnector.name} is already available in Cowork.`
              : connectMessage}
          </div>
          <button
