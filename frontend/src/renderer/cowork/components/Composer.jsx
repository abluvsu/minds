...
export default function Composer({
  ...
  disabled = false,
  metaReadOnly = false,
  // Independent from metaReadOnly: the project is fixed for the life of
  // a task, but the model should stay switchable mid-conversation (the
  // whole point of per-turn model selection). Defaults to metaReadOnly
  // so any caller that doesn't pass this explicitly keeps the old
  // all-or-nothing behavior.
  modelReadOnly = metaReadOnly,
  // Hides the whole meta-row (project AND model). Only ChatView used
  // this historically, back when the model pill was always suppressed
  // there too — now it also passes hideProject so the row still renders
  // for the (interactive) model pill. Kept for any other caller that
  // genuinely wants neither.
  hideMeta = false,
  ...
  hideModel = false,
  // When true, suppress the project pill but keep the model pill.
  // Used on the in-conversation composer (ChatView) — the project is
  // fixed once a task starts, but the model should stay switchable.
  hideProject = false,
