...
// VITE_SKIP_AUTH=true (frontend/src/renderer/.env) bypasses Keycloak for
// local dev — documented in CLAUDE.md but never actually wired up here,
// so the env file silently did nothing and every localhost dev session
// hit the SSO redirect regardless.
const skipAuth = isCloudHosted || import.meta.env.VITE_SKIP_AUTH === 'true';
...
if (skipAuth) {
  // Cloud (Worker session cookie is the auth gate), or local dev with
  // VITE_SKIP_AUTH=true. Render directly, no Keycloak.
