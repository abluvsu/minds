...
// Agent Platform Core
import { AgentRegistry } from './agent-platform/AgentRegistry';
...
const sessionManager = new SessionManager(agentRegistry);
...
function setupIPC() {
  ...
  // ── Agent Platform IPC ──
  ipcMain.handle('agents:getRegistry', () => {
    return agentRegistry.listAgents();
  });

  ipcMain.handle('agents:createSession', (_event, agentId, context) => {
    return sessionManager.createSession(agentId, context);
  });

  ipcMain.handle('agents:killSession', (_event, sessionId) => {
    sessionManager.killSession(sessionId);
  });

  ipcMain.handle('agents:write', (_event, sessionId, data) => {
    globalPtyManager.write(sessionId, data);
  });

  ipcMain.handle('agents:resize', (_event, sessionId, cols, rows) => {
