...
contextBridge.exposeInMainWorld('agents', {
  ...
  createSession: (agentId: string, context: any) => ipcRenderer.invoke('agents:createSession', agentId, context),
  killSession: (sessionId: string) => ipcRenderer.invoke('agents:killSession', sessionId),
  write: (sessionId: string, data: string) => ipcRenderer.invoke('agents:write', sessionId, data),
